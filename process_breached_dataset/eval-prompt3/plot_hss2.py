...
for model, filename in files.items():
    ...
    for col in ["ave_cs_hss_pw1", "ave_ci_hss_pw1", "ave_cs_hss_pw2", "ave_ci_hss_pw2", "ave_cs_hss_user", "ave_ci_hss_user"]:
    ...
    cs_hss_user_data = df["ave_cs_hss_user"].dropna()
    ci_hss_user_data = df["ave_ci_hss_user"].dropna()
    ...
    cs_user_binned = pd.cut(cs_hss_user_data, bins=bins, labels=labels_bin, include_lowest=True)
    ci_user_binned = pd.cut(ci_hss_user_data, bins=bins, labels=labels_bin, include_lowest=True)
    ...
    cs_user_counts = cs_user_binned.value_counts().sort_index()
    ci_user_counts = ci_user_binned.value_counts().sort_index()
    ...
    cs_user_vals = [cs_user_counts.get(lb, 0) for lb in labels_bin]
    ci_user_vals = [ci_user_counts.get(lb, 0) for lb in labels_bin]
    ...
    model_data[model] = {
        ...
        "cs_pw2": cs_pw2_vals,
        "ci_pw2": ci_pw2_vals,
        "cs_user": cs_user_vals,
        "ci_user": ci_user_vals
    }
...
names = {
    ...
    "cs_user": "Case Sensitive HSSuser",
    "ci_user": "Case Insensitive HSSuser"
}

# Order in which to stack the bars (bottom to top)
stack_order = ["cs_pw1", "ci_pw1", "cs_pw2", "ci_pw2", "cs_user", "ci_user"]
...
for i in range(1, len(model_data) + 1):
    ...
    fig.add_annotation(
        ...
        y=5,
        text="Higher HSS/HSSuser Scores",
        showarrow=False,
...
fig.update_layout(
    title={
        'text': '<b>Hybrid Similarity Score (HSS/HSSuser) Distribution for Test Case: Prompt 3</b>',
        'y': 0.98,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top',
...
fig.add_annotation(
    ...
    xref="paper",
    yref="paper",
    text="Note: HSS and HSSuser scores indicate the matching quality between model responses and references.Higher scores (>0.5) generally indicate better matching.",
    showarrow=False,
    font=dict(size=12),
    align="center"
)

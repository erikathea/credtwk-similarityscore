...
func main() {
	...
	outputHeader := []string{"pw1"}
	for i := 1; i <= *numVariants; i++ {
		outputHeader = append(outputHeader, fmt.Sprintf("variant%d", i))
		outputHeader = append(outputHeader, fmt.Sprintf("hybrid_similarity_score%d", i))
	}
	...
	for i, record := range records[1:] {
		...
		for _, variant := range variants {
			...
			similarityScore := hybridSimilarity(pw1, variant, 0.5, 0.5)
			row = append(row, fmt.Sprintf("%.4f", similarityScore))
			if similarityScore <= 0.5 {
				blocklistBuffer.WriteString(variant + "\n")
				blocklistCount++
			}
		}
		row = append(row,
